...
# ==== ユーティリティ ====
def upload_bytes(path: str, data: bytes, content_type: str, public=True) -> str:
...
def upload_json(path: str, obj: dict, public=True) -> str:
...
def gen_image_gemini(prompt: str) -> bytes:
    result = client.models.generate_images(model="imagen-3.0-generate-001", prompt=prompt)
    img_bytes = None
...
button:disabled{opacity:.6;cursor:not-allowed}.muted{color:#666}.row{margin:12px 0}
input[type=range]{width:100%}.result{margin-top:20px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
</style></head><body>
<h1>性格診断 → 馬トークン生成（Gemini）</h1>
<p class="muted">各質問を1〜5で回答（1:いいえ / 5:とても当てはまる）。</p>
<div class="card">
  <div class="row"><b>Q1.</b> 新しい環境でもすぐに馴染める</div>
  <input id="q1" type="range" min="1" max="5" value="3" oninput="v1.textContent=this.value"><span id="v1">3</span>
  <div class="row"><b>Q2.</b> 長時間コツコツ続けるのが得意</div>
  <input id="q2" type="range" min="1" max="5" value="3" oninput="v2.textContent=this.value"><span id="v2">3</span>
  <div class="row"><b>Q3.</b> 困難でも落ち着いて対処できる</div>
  ...
  <div class="row">
    ...
    <input id="hint" placeholder="neon cyberpunk, Tokyo night..." style="width:100%;padding:10px;border:1px solid #ddd;border-radius:10px"/>
  </div>
  <div class="grid">
    <button id="run" onclick="mint()">診断して発行</button>
    <button id="openMy" onclick="openMy()">My Horses</button>
  </div>
...
async function mint(){
  ...
  btn.disabled=true; out.textContent="生成中…（30秒前後かかる場合あり）";
  try{
    const res=await fetch('/mint',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({answers:getAns(),user_hint:document.getElementById('hint').value||""})});
    if(!res.ok) throw new Error('failed:'+res.status);
    const data=await res.json();
    const list=JSON.parse(localStorage.getItem(KEY)||"[]");
    list.push({token_id:data.token_id,name:data.name,image:data.image_url,permalink:data.permalink});
    localStorage.setItem(KEY,JSON.stringify(list));
    out.innerHTML='✅ 発行完了: <a href="'+data.permalink+'" target="_blank">カードを開く</a>';
    renderList();
  }catch(e){ out.textContent='エラー: '+e.message; } finally{ btn.disabled=false; }
}
function renderList(){
  ...
  list.forEach(h=>{
    ...
    div.innerHTML='<img src="'+h.image+'" style="width:100%;border-radius:12px;margin-bottom:8px"/>'+
      '<div><b>'+ (h.name||'Unnamed') +'</b></div><a href="'+h.permalink+'" target="_blank">Open</a>';
    el.appendChild(div);
  });
}
function openMy(){ document.getElementById('list').scrollIntoView({behavior:'smooth'}); }
renderList();
</script>
</body></html>"""
...
img{{max-width:100%;border-radius:12px}}</style></head><body>
<h1>{metadata["name"]}</h1>
